...
# Fungsi untuk membaca harga dari file JSON
def read_pricelist():
    try:
        if not os.path.exists("PRICELIST_DAGING.json"):
            return {
        ...
        with open("PRICELIST_DAGING.json", "r", encoding="utf-8") as f:
            pricelist_data = json.load(f)

        return {
            key.capitalize(): value["harga"]
            for key, value in pricelist_data.items()
        }
...
# Fungsi 1: Menampilkan harga daging dari file JSON
def display_pricelist(pricelist):
    print("\n===== PRICELIST DAGING =====")
    for jenis, harga in pricelist.items():
        print(f"{jenis}: Rp {harga:,.0f}/kg")
    print("===============================\n")

# Fungsi 2: Menghitung diskon dengan Nested IF
...
def main():
    ...
    pricelist = read_pricelist()
    display_pricelist(pricelist)
    
    nama = input("Halo! Silakan masukkan nama Anda: ").strip()
    if not nama:
        nama = "Pelanggan"
    print(f"Selamat datang, {nama} di Toko Daging Meow! Selamat berbelanja😊\n")
